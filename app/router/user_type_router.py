...
from app.recommend.recommend_service import (
    user_id_to_classification_id,
...
@user_type_router.post(
    "/user-type", response_model=GenericResponseDTO[UserTypeResponseDTO]
)
async def create_user_type_by_answers(
    ...
    if request.id is None:
        ...
        recommendNew_ids = await RecommendRepository().get(
            pk=await user_id_to_classification_id(
                user_id=userType.user_id, session=session
            ),
            session=session,
        )
        recommendNew_ids = recommendNew_ids.recommend_article_ids
    ...
    return GenericResponseDTO[UserTypeResponseDTO](
        data=UserTypeResponseDTO(
            userType={
                "ISSUE_FINDER": userType.user_type_issue_finder,
