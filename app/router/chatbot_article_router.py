...
@chatbot_article_router.post("/chatbot/article", response_model=GenericResponseDTO)
async def chatbot_article_detail_(
    ...
    rag_applied_result = await request_rag_applied_openai(
        news_id=request.id, system_prompt=request.prompt, session=session
    )

    return GenericResponseDTO(
        result=True,
        data=rag_applied_result,
        message="Related articles found successfully.",
    )
