...
async def process_generate_article_by_url(
    ...
    ai_result_text = (
        ...
        if isinstance(ai_result.result_text, str)
        else json.dumps(ai_result.result_text)
    )
    ...
    if ai_result_dict.get("category") not in [
        ...
        raise ValueError(
            f"유효하지 않은 카테고리입니다: {ai_result_dict.get('category')}"
        )
    ...
    related_documents_lst = (
        ai_result.related_documents if hasattr(ai_result, "related_documents") else []
    )
    ...
    for doc in related_documents_lst:
        ...
        if related_document_url == url:
            logger.info(
                f"Skipping related document with same URL: {related_document_url}"
            )
            continue
        ...
        await ArticleRelatedDocumentService().save(
            article.id, related_document, session
        )
