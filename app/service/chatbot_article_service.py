...
class RagAppliedResult:
    def __init__(
        self, result_text: str, related_documents: List[Union[Document, dict]]
    ):
    ...
    def to_dict(self) -> Dict:
        ...
        return {"result_text": self.result_text, "related_documents": related_docs}
...
# OpenAI 요청 결과 반환
async def openai_response(openai_api_key: str, prompt: str) -> LLMResult:
    try:
        search_llm = ChatOpenAI(
            temperature=0, model="gpt-4", max_tokens=1500, api_key=openai_api_key
        )
        response = await search_llm.agenerate(messages=[[HumanMessage(prompt)]])
        return response
